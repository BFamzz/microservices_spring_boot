...
waitForService curl http://$HOST:$PORT/products-composite/$PROD_ID_REVS_RECS

# Verify that a normal request works, expect three recommendations and three reviews
assertCurl 200 "curl http://$HOST:$PORT/products-composite/$PROD_ID_REVS_RECS -s"
...
assertCurl 404 "curl http://$HOST:$PORT/products-composite/$PROD_ID_NOT_FOUND -s"
...
assertCurl 200 "curl http://$HOST:$PORT/products-composite/$PROD_ID_NO_RECS -s"
...
assertCurl 200 "curl http://$HOST:$PORT/products-composite/$PROD_ID_NO_REVS -s"
assertEqual $PROD_ID_NO_REVS $(echo $RESPONSE | jq .productId)
assertEqual 3 $(echo $RESPONSE | jq ".recommendations | length")
assertEqual 0 $(echo $RESPONSE | jq ".reviews | length")

# Verify that a 422 (Unprocessable Entity) error is returned for a productId that is out of range (-1)
assertCurl 422 "curl http://$HOST:$PORT/products-composite/-1 -s"
assertEqual "\"Invalid productId: -1\"" "$(echo $RESPONSE | jq .message)"

# Verify that a 400 (Bad Request) error error is returned for a productId that is not a number, i.e. invalid format
assertCurl 400 "curl http://$HOST:$PORT/products-composite/invalidProductId -s"
